...
export const MultilineSelect = withStyles(multilineSelectStyles)(
	(props: MultilineSelectType & WithStyles<typeof multilineSelectStyles>) => {
...
export const FileUploadLabel = withStyles((theme: Theme) =>
	createStyles({
		root: {
			alignItems: 'center',
			cursor: 'pointer',
...
export const FileView = withStyles(fileViewStyles)(
	({
		classes,
		file,
		onClearForm,
		errors = []
	}: FileViewProps & WithStyles<typeof fileViewStyles>) => (
		<Grid
			item
			className={classNames(
				classes.fileItem,
				errors && errors.length && classes.fileItemError
			)}
		>
			<Grid
				container
...
const isAudioType = (mime: string) =>
	['audio/ogg', 'audio/mp3', 'audio/m4a', 'audio/x-wav'].includes(mime);
	...
	render() {
		...
		return (
			<Grid
				item
				className={
					dragAndDrop ? classes.fileViewContainerDragAndDrop : classes.fileViewContainer
				}
			>
				<Grid
					container
...
export const FileUploadWidget = withStyles(fileUploadStyles)(
	({
		...
		required,
		...props
	}: FileUploadWidgetProps & WithStyles<typeof fileUploadStyles>) => {
...
const FileUploadGrid = withStyles((theme: Theme) =>
	createStyles({
		container: {
...
class ArrayFileUploadWidgetComponent extends React.Component<ArrayFileUploadWidgetProps> {
	...
	handleFormFileChange = (evt: any) => {
		if (this.props.onChange) this.props.onChange(Array.from(evt.target.files), true);
	};
...
export const ArrayFileUploadWidget = withStyles(fileUploadWidgetStyles)(
	ArrayFileUploadWidgetComponent
);
