...
export class CryptoChartBoxComponent extends React.Component<StyledProps, CryptoChartBoxState> {
	...
	getTokensLegend(classes: any, tokens: Token[], locale: string, fiatCurrency: string) {
	...
	getChartData(tokens: Token[]) {
		const data: (string | number)[][] = [['Content', 'percents']];
		let dataPoints = null;
		if (this.hasBalance()) {
	...
	getTotalBalanceInFiat(tokens: Token[]) {
